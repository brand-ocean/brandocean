...
export default defineSchema({
	...
	specs: defineTable({
		...
		shareToken: v.string(),
		published: v.boolean(),
		// Dezelfde tabel draagt twee soorten lijsten. "vragen" is het origineel:
		// ik vraag, de klant antwoordt. "status" draait het om: elke regel is een
		// stuk werk en het vinkje betekent af in plaats van beantwoord. Leeg is
		// "vragen", zodat de bestaande lijsten niets merken.
		kind: v.optional(v.union(v.literal("vragen"), v.literal("status"))),
		createdAt: v.number(),
		updatedAt: v.number(),
	})
		.index("by_slug", ["slug"])
	...
	// `note` is de enige regel die van mij komt en niet van de klant: wat ik met
	// het antwoord heb gedaan. Daarom staat hij los van `answer`, zodat zijn
	// woorden zijn woorden blijven.
	...
	// --- Bellen: de klusjes waar ik zelf niet meer voor in de wacht wil ------
	//
	// Dit gaat niet over klanten. Dit is de klantenservice-kant van een BV
	// runnen: een provider die een storing moet inplannen, een leverancier die
	// een factuur crediteert, een verzekeraar die een polis aanpast. Werk dat
	// vooral uit wachten bestaat. De agent belt, hangt in de wachtrij, voert het
	// gesprek, en haalt mij erbij zodra er iets beslist of goedgekeurd moet
	// worden.
	//
	// Twee wetten zitten hier in het ontwerp, niet in een README. De AI Act
	// (art. 50, sinds 2 aug 2026) eist dat de agent hoorbaar zegt dat hij een
	// machine is en namens wie hij belt — dat staat vast in de openingszin en is
	...
	voiceTasks: defineTable({
		ownerId: v.id("users"),
		company: v.string(),
		toNumber: v.string(), // E.164: +31...
		// Wat er moet gebeuren, in mijn eigen woorden. Dit gaat één op één de
		// prompt in, dus schrijf het zoals je het tegen een stagiair zegt.
		goal: v.string(),
		// De grens. Alles hierbuiten is een handoff, geen besluit van de agent.
		constraints: v.optional(v.string()),
		// Gegevens die de agent bij verificatie mág noemen. Ze staan hier en niet
		// in de prompt, omdat de agent ze per stuk moet opvragen via de fact-tool.
		// Zo staat in de log precies wat er is prijsgegeven, en aan wie.
		facts: v.array(
			v.object({
				label: v.string(), // "klantnummer", "postcode", "geboortedatum"
				value: v.string(),
		...
		status: v.union(
			v.literal("draft"),
			v.literal("calling"),
			v.literal("needs_me"), // agent vroeg om een mens
			v.literal("done"),
			v.literal("failed"),
		),
		// Wat de agent aan het eind zegt dat er is gebeurd, plus mijn eigen
		// oordeel daarover. `outcome` komt van de agent en is dus een claim,
		// geen feit — daarom staat het transcript ernaast.
	...
	// Eén poging. Een taak heeft er vaak meer dan één: in de wacht, weggedrukt,
	// "belt u morgen even terug". `token` is de sleutel waarmee de agent tijdens
	// dit ene gesprek bij de tools mag — hij leeft zo lang als het gesprek en
	// geeft toegang tot precies deze taak, niet tot de rest van de database.
	voiceCalls: defineTable({
		taskId: v.id("voiceTasks"),
		ownerId: v.id("users"),
		token: v.string(),
		...
		// Door de agent gevraagd omdat er een mens bij moest. De reden is zijn
		// eigen formulering.
