...
export const getBySlug = query({
	...
	handler: async (
		...
		return {
			title: spec.title,
			intro: spec.intro,
			kind: spec.kind ?? "vragen",
			// Bewust niet het hele document terug: de publieke pagina heeft geen
			// eigenaar, token of interne velden nodig. `resolved` gaat wel mee: dat
			// is het vinkje waaraan de klant ziet welke vragen rond zijn, zodat hij
			// alleen de rest hoeft aan te vullen. `note` ook: dat is wat wij met zijn
			// antwoord hebben gedaan, en juist dat wil hij terugzien.
