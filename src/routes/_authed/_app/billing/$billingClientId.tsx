...
const RESOURCE_KINDS = [
	...
	{ value: "cx_deployment", label: "Convex deployment", hint: "deployment-naam" },
] as const;
...
function formatQuantity(unit: string, qty: number): string {
	...
	return new Intl.NumberFormat("nl-NL", { maximumFractionDigits: 0 }).format(qty);
}
...
function MandateCard({ id, record }: { id: Id<"billingClients">; record: Record }) {
		...
		<Card>
			...
			<CardContent className="flex flex-wrap items-center justify-between gap-4">
				...
				{!valid && (
					...
					>
						{starting ? "Bezig…" : record.hasMandate ? "Opnieuw proberen" : "Incasso starten"}
					</Button>
				)}
			</CardContent>
		</Card>
		...
		<Card>
			...
			<CardContent>
				{resources === undefined ? (
					...
					<p className="text-sm text-muted-foreground">
						Nog niks gekoppeld. Voeg minstens één Worker, zone of deployment toe.
					</p>
				) : (
...
function AddResourceDialog({ id }: { id: Id<"billingClients"> }) {
	...
	const [kind, setKind] = useState<(typeof RESOURCE_KINDS)[number]["value"]>(
		"cf_worker",
	);
					...
					<div className="space-y-4">
						...
						<div className="space-y-1 border-t pt-4 text-sm">
							<div className="flex justify-between">
								<span className="text-muted-foreground">Verbruik deze periode</span>
							...
							{usage.projectedTotalCents < usage.minChargeCents && (
								<p className="pt-1 text-xs text-muted-foreground">
									Onder het minimum van {eur(usage.minChargeCents, usage.currency)}
									 — schuift door naar de volgende periode.
								</p>
							)}
						</div>
					</div>
					...
					<Table>
						...
						<TableBody>
							{invoices.map((inv) => {
								...
								const amount =
									inv.status === "carried" ? inv.carryOutCents : inv.chargedCents;
								return (
...
function SettingsDialog({
	...
	const [interval, setInterval] = useState(String(record.billingIntervalMonths));
	const [minEuro, setMinEuro] = useState((record.minChargeCents / 100).toFixed(2));
	const [saving, setSaving] = useState(false);
		...
		>
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Marge, incasso-interval en de minimumdrempel voor {record.clientName}.
					</DialogDescription>
				</DialogHeader>
				<form
					onSubmit={async (e) => {
